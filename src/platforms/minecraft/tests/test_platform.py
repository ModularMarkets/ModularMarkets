...
from .platform import MinecraftBot, MinecraftBotNet, Minecraft, MinecraftInventory
...
class TestMinecraftBot:
    ...
    def test_bot_initialization(self, bot):
        ...
        assert isinstance(bot.inventory, MinecraftInventory)
        assert bot.inventory.capacity == 36 * 64
    ...
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_clean_inventory_drop_mode(self, mock_client_class, bot):
        """Test _clean_inventory drops items not in stored_item_types."""
        bot.trading_mode = 'drop'
        bot.stored_item_types = ['diamond', 'emerald']
        ...
        mock_client.drop_items.return_value = {
            'success': True,
            'dropped_count': 2
        }
        
        result = bot._clean_inventory()
        
        assert result == 0
        mock_client.drop_items.assert_called_once()
        call_args = mock_client.drop_items.call_args
        assert call_args[0][0] == bot.bot_id
        assert 'diamond' in call_args[0][1]
        assert 'emerald' in call_args[0][1]
    
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_clean_inventory_with_additional_items(self, mock_client_class, bot):
        """Test _clean_inventory with additional allowed items."""
        bot.trading_mode = 'drop'
        bot.stored_item_types = ['diamond']
        ...
        mock_client.drop_items.return_value = {'success': True}
        
        result = bot._clean_inventory(additional_allowed_items=['emerald'])
        
        assert result == 0
        call_args = mock_client.drop_items.call_args[0]
        allowed_items = call_args[1]
        assert 'diamond' in allowed_items
        assert 'emerald' in allowed_items
    
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_clean_inventory_bot_not_connected(self, mock_client_class, bot):
        """Test _clean_inventory returns error when bot not connected."""
        bot.trading_mode = 'drop'
    ...
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_clean_inventory_not_applicable_mode(self, mock_client_class, bot):
        """Test _clean_inventory returns 0 for non-drop/chat modes."""
        bot.trading_mode = 'plugin'
    ...
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_retrieve_item_drop_mode_full_flow(self, mock_client_class, bot):
        """Test retrieve_item for drop mode - full flow."""
        bot.trading_mode = 'drop'
        bot._server_address = 'localhost'
        bot._server_port = 25565
        bot._minecraft_version = '1.21.4'
        bot._db_session = Mock()
        ...
        mock_client.validate_inventory.return_value = {
            'success': True,
            'is_accurate': True,
            'actual_inventory': {},
            'differences': {}
        }
        mock_client.drop_items.return_value = {'success': True, 'dropped_count': 0}
        mock_client.send_chat.return_value = {'success': True}
        mock_client.wait_for_items.return_value = {
            'success': True,
            'received_amount': 10,
            'target_amount': 10
        }
        ...
        mock_client.logout.return_value = {'success': True}
        
        result = bot.retrieve_item('diamond', 10, 'test_uuid')
        
        assert result == 0
        mock_client.send_chat.assert_called_once()
        assert 'seeking diamond, 10' in mock_client.send_chat.call_args[0][1]
        mock_client.wait_for_items.assert_called_once()
        assert mock_client.wait_for_items.call_args[1]['item_name'] == 'diamond'
        assert mock_client.wait_for_items.call_args[1]['target_amount'] == 10
        mock_client.logout.assert_called_once()
    
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_retrieve_item_72_diamonds(self, mock_client_class, bot):
        """Test retrieve_item specifically for 72 diamonds."""
        bot.trading_mode = 'drop'
        bot._server_address = 'localhost'
        bot._server_port = 25565
        bot._minecraft_version = '1.21.4'
        ...
        # Mock all API calls for 72 diamonds
        mock_client.get_status.return_value = {
            'success': True,
            'status': 'connected'
        }
        ...
        mock_client.get_inventory.return_value = {
            'success': True,
            'inventory': {'diamond': 72}
        }
        mock_client.logout.return_value = {'success': True}
        
        result = bot.retrieve_item('diamond', 72, 'test_uuid')
        
        assert result == 0
        # Verify chat message mentions 72 diamonds
        chat_call = mock_client.send_chat.call_args
        assert 'seeking diamond, 72' in chat_call[0][1]
        # Verify wait_for_items was called with 72
        wait_call = mock_client.wait_for_items.call_args
        assert wait_call[1]['target_amount'] == 72
        assert wait_call[1]['item_name'] == 'diamond'
        # Verify inventory was updated
        assert bot.inventory.items.get('diamond', 0) == 72
        assert 'diamond' in bot.stored_item_types
    
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_retrieve_item_not_drop_mode(self, mock_client_class, bot):
        """Test retrieve_item returns error for non-drop modes."""
        bot.trading_mode = 'chat'
        
        result = bot.retrieve_item('diamond', 10, 'test_uuid')
        
        assert result == 1  # Not implemented for chat yet
        mock_client_class.assert_not_called()
    
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_retrieve_item_insufficient_amount(self, mock_client_class, bot):
        """Test retrieve_item returns error when insufficient items received."""
        bot.trading_mode = 'drop'
        bot._server_address = 'localhost'
        bot._server_port = 25565
        bot._minecraft_version = '1.21.4'
        ...
        mock_client.drop_items.return_value = {'success': True}
        mock_client.send_chat.return_value = {'success': True}
        # Only received 5 out of 10 requested
        mock_client.wait_for_items.return_value = {
            'success': True,
            'received_amount': 5,
            'target_amount': 10
        ...
        assert result == 1  # Error: didn't receive enough
        mock_client.logout.assert_called()  # Should still logout on error
    
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_retrieve_item_bot_login_on_disconnect(self, mock_client_class, bot):
        """Test retrieve_item logs in bot if not connected."""
        bot.trading_mode = 'drop'
        bot._server_address = 'test.server.com'
        bot._server_port = 25566
        bot._minecraft_version = '1.21.4'
        
        mock_client = Mock()
        mock_client_class.return_value = mock_client
        
        # Bot not connected initially
        mock_client.get_status.return_value = {
            'success': True,
            'status': 'disconnected'
        }
        ...
        # After login, bot is connected
        mock_client.get_status.side_effect = [
            {'success': True, 'status': 'disconnected'},  # First check
            {'success': True, 'status': 'connected'}      # After login
        ]
        mock_client.validate_inventory.return_value = {'success': True, 'is_accurate': True}
        mock_client.drop_items.return_value = {'success': True}
        mock_client.send_chat.return_value = {'success': True}
        mock_client.wait_for_items.return_value = {
            'success': True,
        ...
        login_call = mock_client.login.call_args
        assert login_call[1]['bot_id'] == bot.bot_id
        assert login_call[1]['username'] == bot.username
        assert login_call[1]['server_host'] == 'test.server.com'
        assert login_call[1]['server_port'] == 25566
...
class TestMinecraftBotNet:
    ...
    def test_get_warehouse_for_retrieve_prioritizes_bots_with_item(self, network):
        """Test get_warehouse_for_retrieve prioritizes bots set to store item."""
        from .platform import MinecraftInventory
        
        bot1 = MinecraftBot("bot1", "pass1", "offline", "drop", "bot1")
        bot1.stored_item_types = ['diamond']
        bot1.inventory = MinecraftInventory()
        bot1.inventory.items = {'diamond': 5}
        
        bot2 = MinecraftBot("bot2", "pass2", "offline", "drop", "bot2")
        bot2.stored_item_types = []
        bot2.inventory = MinecraftInventory()
        bot2.inventory.items = {}
        
        network.warehouses = [bot1, bot2]
        
        # Mock item weight getter
        def get_weight(name):
            return 1
        
        bot1.inventory._get_item_weight = get_weight
        bot2.inventory._get_item_weight = get_weight
        
        result = network.get_warehouse_for_retrieve('diamond', 10)
        
        assert result == bot1  # Should prefer bot1 since it stores diamond
    
    def test_get_warehouse_for_retrieve_least_items(self, network):
        """Test get_warehouse_for_retrieve selects bot with least items."""
        from .platform import MinecraftInventory
        
        bot1 = MinecraftBot("bot1", "pass1", "offline", "drop", "bot1")
        bot1.inventory = MinecraftInventory()
        bot1.inventory.items = {'diamond': 50}
        
        bot2 = MinecraftBot("bot2", "pass2", "offline", "drop", "bot2")
        bot2.inventory = MinecraftInventory()
        bot2.inventory.items = {'emerald': 10}
        ...
        result = network.get_warehouse_for_retrieve('gold_ingot', 10)
        
        assert result == bot2  # bot2 has fewer items
    
    def test_get_warehouse_for_retrieve_no_capacity(self, network):
        """Test get_warehouse_for_retrieve raises error when no bot can fit."""
        from .platform import MinecraftInventory
        
        bot = MinecraftBot("bot1", "pass1", "offline", "drop", "bot1")
        bot.inventory = MinecraftInventory(capacity=100)  # Small capacity
        bot.inventory.items = {}
        
        network.warehouses = [bot]
        
        def get_weight(name):
            return 1
        
        bot.inventory._get_item_weight = get_weight
        
        with pytest.raises(ValueError, match="No bot in network can fit"):
            network.get_warehouse_for_retrieve('diamond', 200)  # Too much
    
    def test_get_warehouse_for_retrieve_empty_network(self, network):
        """Test get_warehouse_for_retrieve raises error with no bots."""
        with pytest.raises(ValueError, match="No bots available"):
            network.get_warehouse_for_retrieve('diamond', 10)
    
    def test_get_warehouse_for_retrieve_72_diamonds(self, network):
        """Test get_warehouse_for_retrieve specifically for 72 diamonds."""
        from .platform import MinecraftInventory
        
        bot1 = MinecraftBot("bot1", "pass1", "offline", "drop", "bot1")
        bot1.inventory = MinecraftInventory()
        bot1.inventory.items = {'emerald': 20}
        
        bot2 = MinecraftBot("bot2", "pass2", "offline", "drop", "bot2")
        bot2.inventory = MinecraftInventory()
        bot2.inventory.items = {'gold_ingot': 5}
        ...
        result = network.get_warehouse_for_retrieve('diamond', 72)
        
        # Should select bot with least items (bot2)
        assert result == bot2
        # Verify bot can handle 72 diamonds
        can_add = bot2.inventory.amount_of_item_that_can_be_added('diamond')
        assert can_add >= 72
...
class TestMinecraftPlatform:
    ...
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_retrieve_item_drop_mode(self, mock_client_class):
        """Test retrieve_item delegates to network and bot."""
        platform = Mock(spec=Minecraft)
        platform._network = Mock()
        ...
        # Mock bot
        mock_bot = Mock()
        mock_bot.retrieve_item.return_value = 0
        platform._network.get_warehouse_for_retrieve.return_value = mock_bot
        
        result = platform.retrieve_item("diamond", 10, "uuid123")
        
        assert result == 0
        platform._network.get_warehouse_for_retrieve.assert_called_once_with("diamond", 10)
        mock_bot.retrieve_item.assert_called_once_with("diamond", 10, "uuid123")
    
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_retrieve_item_72_diamonds_platform_level(self, mock_client_class):
        """Test retrieve_item at platform level specifically for 72 diamonds."""
        platform = Mock(spec=Minecraft)
        platform._network = Mock()
        platform.retrieve_item = Minecraft.retrieve_item.__get__(platform, Minecraft)
        ...
        result = platform.retrieve_item("diamond", 72, "uuid123")
        
        assert result == 0
        # Verify network was called with 72 diamonds
        platform._network.get_warehouse_for_retrieve.assert_called_once_with("diamond", 72)
        # Verify bot was called with 72 diamonds
        mock_bot.retrieve_item.assert_called_once_with("diamond", 72, "uuid123")
    
    @patch('src.platforms.minecraft.node_service.MineflayerClient')
    def test_retrieve_item_retry_on_insufficient_capacity(self, mock_client_class):
        """Test retrieve_item retries with different bot if first can't fit."""
        platform = Mock(spec=Minecraft)
        platform._network = Mock()
        platform.retrieve_item = Minecraft.retrieve_item.__get__(platform, Minecraft)
        
        # First bot can't handle the amount
        mock_bot1 = Mock()
        mock_bot1.retrieve_item.side_effect = ValueError("Cannot fit")
        
        # Second bot succeeds
        mock_bot2 = Mock()
        mock_bot2.retrieve_item.return_value = 0
        
        platform._network.get_warehouse_for_retrieve.side_effect = [mock_bot1, mock_bot2]
        
        result = platform.retrieve_item("diamond", 100, "uuid123")
        
        assert result == 0
        # Should have tried twice
        assert platform._network.get_warehouse_for_retrieve.call_count == 2
        mock_bot2.retrieve_item.assert_called_once_with("diamond", 100, "uuid123")
